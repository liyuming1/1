...
function renderAccounts() {
    ...
    document.querySelectorAll('.btn-config').forEach(btn => {
        btn.addEventListener('click', (e) => {
            ...
            form.autoTask.checked = acc.config?.autoTask !== false;
            form.autoSteal.checked = acc.config?.autoSteal !== false;
            form.autoHelp.checked = acc.config?.autoHelp !== false;
...
function renderLands(landsData) {
    ...
    for (let i = 0; i < 24; i++) {
        ...
        if (!land || !land.unlocked) {
            ...
            cell.innerHTML = '<span class="land-icon">⬛</span><span class="land-name">锁定</span>';
        } else if (!land.plant) {
            cell.className = 'land-cell empty';
            cell.innerHTML = '<span class="land-icon">⬜</span><span class="land-name">空地</span>';
        } else {
...
function setupEventListeners() {
    ...
    document.getElementById('form-config').addEventListener('submit', async (e) => {
        ...
        const config = {
            ...
            friendInterval: parseInt(form.friendInterval.value),
            autoSell: form.autoSell.checked,
            autoFarm: form.autoFarm.checked,
            autoFertilize: form.autoFertilize.checked,
            autoTask: form.autoTask.checked,
            autoSteal: form.autoSteal.checked,
            autoHelp: form.autoHelp.checked,
