...
let currentRestartAccountId = null;
...
async function checkQrAndStartAccount(ownerId = null) {
    ...
    if (result.status === 'OK') {
        ...
        // 如果是重启流程，更新现有账号
        if (currentRestartAccountId) {
            account = getAccount(currentRestartAccountId);
            if (account) {
            ...
            currentRestartAccountId = null;
        }
        
        // 如果没有找到现有账号，创建新账号
        if (!account) {
            account = addAccount(`账号${accounts.length + 1}`, 'qq', currentQrOwnerId || ownerId);
            account.code = result.code;
            account.enabled = true;
            account.status.startTime = Date.now();
            account.name = '扫码成功，启动中...';
...
function setRestartAccountId(id) {
    currentRestartAccountId = id;
}
