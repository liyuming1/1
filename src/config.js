...
function parseGrowPhases(plantData) {
    ...
    const phaseToName = {};
    const parts = growPhasesStr.split(";");
    let phaseIndex = 1; // 游戏中的phase从1开始
    for (const part of parts) {
        if (!part) continue;
        const [name, duration] = part.split(":");
        if (name) {
            phaseToName[phaseIndex] = name;
            phaseIndex++;
        }
    }
    return phaseToName;
...
function getPlantPhaseNames(plantIdOrSeedId) {
    // 先尝试直接用ID查找 (plantId)
    if (PLANT_PHASE_MAP.has(plantIdOrSeedId)) {
        return PLANT_PHASE_MAP.get(plantIdOrSeedId);
    }
    // 再尝试用seed_id查找
    if (PLANT_SEED_TO_PHASE_MAP.has(plantIdOrSeedId)) {
        return PLANT_SEED_TO_PHASE_MAP.get(plantIdOrSeedId);
    }
    return null;
}
