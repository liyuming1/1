...
const plantData = require("../gameConfig/Plant.json");

function parseGrowPhases(plantData) {
    const growPhasesStr = plantData.grow_phases;
    if (!growPhasesStr) return null;
    
    const phaseToName = {};
    const parts = growPhasesStr.split(";").filter(p => p);
    ...
    for (let i = 0; i < parts.length; i++) {
        ...
        phaseToName[i + 1] = name;
    }
    return phaseToName;
}

// 获取植物各阶段的时长（用于按生长秒数计算阶段）
function getPlantPhaseDurations(plantIdOrSeedId) {
    const numId = Number(plantIdOrSeedId);
    
    // 直接从 Plant.json 解析
    if (plantData && Array.isArray(plantData)) {
        for (const row of plantData) {
            if (row.id === numId || row.seed_id === numId) {
                const growPhasesStr = row.grow_phases;
                if (!growPhasesStr) return null;
                ...
                for (const part of parts) {
                    const colonIndex = part.indexOf(":");
                    if (colonIndex === -1) continue;
                    const duration = parseInt(part.substring(colonIndex + 1));
                    // 过滤掉 duration=0 的项
...
const PLANT_PHASE_MAP = new Map();
const PLANT_SEED_TO_PHASE_MAP = new Map();
if (plantData && Array.isArray(plantData)) {
    for (const row of plantData) {
        const plantId = row.id;        // 如 1020044
        const seedId = row.seed_id;    // 如 20044
        const phaseToName = parseGrowPhases(row);
        if (phaseToName && Object.keys(phaseToName).length > 0) {
            // 用 plantId (1020044) 建立映射
            PLANT_PHASE_MAP.set(plantId, phaseToName);
            // 用 seedId (20044) 建立映射
            if (seedId) {
                PLANT_SEED_TO_PHASE_MAP.set(seedId, phaseToName);
            }
        }
    }
}

function getPlantPhaseNames(plantIdOrSeedId) {
    const numId = Number(plantIdOrSeedId);
    let result = null;
    if (PLANT_PHASE_MAP.has(numId)) {
        result = PLANT_PHASE_MAP.get(numId);
    } else if (PLANT_SEED_TO_PHASE_MAP.has(numId)) {
        result = PLANT_SEED_TO_PHASE_MAP.get(numId);
    }
    return result;
}
