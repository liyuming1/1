...
if (plantData && Array.isArray(plantData)) {
    for (const row of plantData) {
        ...
        const growPhases = parseGrowPhases(row);
        if (growPhases && growPhases.length > 0) {
            PLANT_PHASE_MAP.set(plantId, growPhases);
            if (seedId) {
                PLANT_SEED_TO_PHASE_MAP.set(seedId, growPhases);
            }
        }
    }
}
