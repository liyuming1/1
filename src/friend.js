...
const { toLong, toNum, getServerTimeSec, log, logWarn, sleep, getCurrentAccountId } = require('./utils');
...
function getSeedRequiredLevel(plantId) {
    const seedId = plantId - 1020000 + 20000;
    const seed = (seedsData.rows || []).find(s => s.seedId === seedId);
    return seed ? (seed.requiredLevel || 0) : 0;
}
    ...
    for (const land of lands) {
        ...
        if (phaseVal === PlantPhase.MATURE) {
            if (plant.stealable) {
                ...
                const requiredLevel = getSeedRequiredLevel(plantId);
                if (localFriendConfig.stealMinLevel <= 0 || requiredLevel >= localFriendConfig.stealMinLevel) {
...
async function visitFriend(friend, totalActions, myGid) {
    ...
    // 帮助好友: 根据开关执行 (除草/除虫/浇水)
    if (localFriendConfig.autoHelp) {
        if (status.needWeed.length > 0 && (!HELP_ONLY_WITH_EXP || canGetExp(10005))) {
            markExpCheck(10005);
            let ok = 0;
        ...
        if (status.needBug.length > 0 && (!HELP_ONLY_WITH_EXP || canGetExp(10006))) {
            markExpCheck(10006);
            let ok = 0;
        ...
        if (status.needWater.length > 0 && (!HELP_ONLY_WITH_EXP || canGetExp(10007))) {
            markExpCheck(10007);
            let ok = 0;
    ...
    // 偷菜: 根据开关执行
    if (status.stealable.length > 0 && localFriendConfig.autoSteal) {
        let ok = 0;
        const stolenPlants = [];
    ...
    // 捣乱操作: 根据开关执行
    if (localFriendConfig.autoPutBug && status.canPutBug.length > 0 && canOperate(10004)) {
        let ok = 0;
    ...
    if (localFriendConfig.autoPutWeed && status.canPutWeed.length > 0 && canOperate(10003)) {
        let ok = 0;
    ...
    try {
        ...
        for (const f of friends) {
            ...
            if (hasSteal) {
                ...
                visitedGids.add(gid);
            } else if ((localFriendConfig.autoPutBug || localFriendConfig.autoPutWeed) && canPutBugOrWeed) {
...
async function friendCheckLoop() {
    while (friendLoopRunning) {
        ...
        if (currentId) {
            ...
            if (accountConfig) {
                if (accountConfig.friendCheckInterval !== undefined) {
                    localFriendConfig.friendCheckInterval = accountConfig.friendCheckInterval * 1000;
                }
                if (accountConfig.autoSteal !== undefined) localFriendConfig.autoSteal = accountConfig.autoSteal;
                if (accountConfig.autoHelp !== undefined) localFriendConfig.autoHelp = accountConfig.autoHelp;
                if (accountConfig.autoPutBug !== undefined) localFriendConfig.autoPutBug = accountConfig.autoPutBug;
                if (accountConfig.autoPutWeed !== undefined) localFriendConfig.autoPutWeed = accountConfig.autoPutWeed;
                if (accountConfig.stealMinLevel !== undefined) localFriendConfig.stealMinLevel = accountConfig.stealMinLevel;
            }
        }
        
        await checkFriends();
        if (!friendLoopRunning) break;
        await sleep(localFriendConfig.friendCheckInterval);
    }
}
