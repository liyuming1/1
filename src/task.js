...
function initTaskSystem() {
    if (CONFIG.autoClaim === false) {
        console.log('[任务] autoClaim 已关闭，跳过初始化');
        return;
    }
...
async function claimFreeGifts() {
    try {
        const body = types.GetMallGoodsListRequest.encode(types.GetMallGoodsListRequest.create({})).finish();
        const { body: replyBody } = await sendMsgAsync('gamepb.mallpb.MallService', 'GetMallGoodsList', body);
        const reply = types.GetMallGoodsListReply.decode(replyBody);
        // 领取免费礼包逻辑
        log('奖励', '免费礼包检查完成');
    } catch (e) {
...
async function claimShareReward() {
    try {
        const body = types.GetShareRewardRequest.encode(types.GetShareRewardRequest.create({})).finish();
        const { body: replyBody } = await sendMsgAsync('gamepb.taskpb.TaskService', 'GetShareReward', body);
        log('奖励', '分享奖励检查完成');
    } catch (e) {
...
async function claimMonthCard() {
    try {
        const body = types.GetMonthCardRequest.encode(types.GetMonthCardRequest.create({})).finish();
        const { body: replyBody } = await sendMsgAsync('gamepb.taskpb.TaskService', 'GetMonthCard', body);
        log('奖励', '月卡奖励检查完成');
    } catch (e) {
...
async function claimEmailReward() {
    try {
        const body = types.GetEmailListRequest.encode(types.GetEmailListRequest.create({})).finish();
        const { body: replyBody } = await sendMsgAsync('gamepb.mailpb.MailService', 'GetEmailList', body);
        log('奖励', '邮箱奖励检查完成');
    } catch (e) {
...
async function claimVipGift() {
    try {
        const body = types.GetVipGiftRequest.encode(types.GetVipGiftRequest.create({})).finish();
        const { body: replyBody } = await sendMsgAsync('gamepb.taskpb.TaskService', 'GetVipGift', body);
        log('奖励', 'QQ会员礼包检查完成');
    } catch (e) {
...
async function claimIllustrated() {
    try {
        const body = types.GetIllustratedRequest.encode(types.GetIllustratedRequest.create({})).finish();
        const { body: replyBody } = await sendMsgAsync('gamepb.taskpb.TaskService', 'GetIllustrated', body);
        log('奖励', '图鉴奖励检查完成');
    } catch (e) {
