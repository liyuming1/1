...
const { toLong, toNum, getServerTimeSec, toTimeSec, log, logWarn, sleep, getCurrentAccountId } = require('./utils');
const { getPlantNameBySeedId, getPlantName, getPlantExp, formatGrowTime, getPlantGrowTime, getPlantPhaseNames, getPlantByName, getItemName, getLevelExpProgress } = require('./gameConfig');
...
async function getAllLands(hostGid = 0) {
    ...
    const targetGid = hostGid || (state.gid || 0);
    const body = types.AllLandsRequest.encode(types.AllLandsRequest.create({
        host_gid: toLong(targetGid),
    })).finish();
...
async function findBestSeed(landsCount) {
    ...
    const accountConfig = currentAccountId ? webUI.accountManager.getAccountConfig(currentAccountId) : null;
    const configuredSeedId = accountConfig?.seedId;

    if (configuredSeedId) {
        const selected = available.find(x => x.seedId === configuredSeedId);
        if (selected) {
            log('商店', `使用配置的种子: ${selected.seedId}`);
            return selected;
        }
    }
    ...
    if(state.level && state.level <= 28){
        ...
        available.sort((a, b) => b.requiredLevel - b.requiredLevel);
    }
    return available[0];
}
...
async function autoPlantEmptyLands(deadLandIds, emptyLandIds, unlockedLandCount) {
    ...
    // 5. 施肥（逐块拖动，间隔50ms）- 根据自动开关
    // 需求：一次种植循环里，先普通肥，再有机肥
    if (plantedLands.length > 0 && localFarmConfig.autoFertilize) {
        // 先普通肥
...
function analyzeLands(lands) {
    ...
    for (const land of lands) {
        ...
        if (maturePhase) {
            let matureBegin = 0;
            if (maturePhase.begin_time && maturePhase.begin_time.low !== undefined) {
                matureBegin = maturePhase.begin_time.low;
            } else {
                matureBegin = toTimeSec(maturePhase.begin_time);
            }
...
function buildLandDetails(lands) {
    const details = [];
    const nowSec = getServerTimeSec();
    ...
    for (const land of lands) {
        ...
        // 判断是否成熟：最后一个阶段就是成熟阶段（每个植物阶段数不同）
        const lastPhase = plant.phases && plant.phases.length > 0 ? plant.phases[plant.phases.length - 1] : null;
        const maturePhaseVal = lastPhase ? lastPhase.phase : 6; // 成熟阶段的phase值
        const isMature = phaseVal === maturePhaseVal;
        ...
        if (isMature) {
            ...
            // 计算距成熟剩余时间 - 使用最后一个阶段作为成熟阶段
            const lastPhase = plant.phases[plant.phases.length - 1];
            let secs = -1;
            
            if (lastPhase) {
                let matureBegin = 0;
                if (lastPhase.begin_time && typeof lastPhase.begin_time.low === 'number') {
                    matureBegin = lastPhase.begin_time.low;
                } else {
                    matureBegin = toTimeSec(lastPhase.begin_time);
                }
                secs = matureBegin - nowSec;
...
async function checkFarm() {
    ...
    try {
        ...
        // 广播账号状态给前端
        const landDetails = buildLandDetails(lands);
        ...
        if (currentId) {
            ...
            const runTime = account && account.status.startTime ? Math.floor((Date.now() - account.status.startTime) / 1000) : 0;
            
            webUI.broadcastAccountStatus(currentId, {
                level: state.level,
                currentExp: expProgress.current,
                neededExp: expProgress.needed,
                gold: state.gold,
                runTime: runTime,
                landsCount: unlockedLandCount,
                lands: landDetails
            });
            accountManager.updateAccountStatus(currentId, { 
                lands: landDetails,
                level: state.level,
                exp: state.exp,
                gold: state.gold
        ...
        if (status.harvestable.length > 0 && localFarmConfig.autoFarm) {
            try {
        ...
        // 铲除 + 种植 + 施肥（需要顺序执行）- 根据自动开关
        const allDeadLands = [...status.dead, ...harvestedLandIds];
        const allEmptyLands = [...status.empty];
        if ((allDeadLands.length > 0 || allEmptyLands.length > 0) && localFarmConfig.autoFarm) {
            try {
...
async function farmCheckLoop() {
    while (farmLoopRunning) {
        ...
        const currentId = getCurrentAccountId();
        if (currentId) {
            const accountConfig = webUI.accountManager.getAccountConfig(currentId);
            if (accountConfig) {
                if (accountConfig.farmCheckInterval !== undefined) {
                    localFarmConfig.farmCheckInterval = accountConfig.farmCheckInterval * 1000;
                }
                if (accountConfig.seedId !== undefined) {
                    localFarmConfig.seedId = accountConfig.seedId;
                }
                if (accountConfig.autoFarm !== undefined) {
                    localFarmConfig.autoFarm = accountConfig.autoFarm;
                }
                if (accountConfig.autoFertilize !== undefined) {
                    localFarmConfig.autoFertilize = accountConfig.autoFertilize;
                }
            }
        }
        ...
        await sleep(localFarmConfig.farmCheckInterval);
    }
}
