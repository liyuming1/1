...
function serializeLands(landsReply) {
    ...
    return {
        ...
        lands: lands.map(land => {
            ...
            // 计算当前阶段
            const currentPhase = plant?.phases ? getCurrentPhase(plant.phases, false, landLabel, plantId, growSec) : null;
...
function getCurrentPhase(phases, debug, landLabel, plantId, growSec) {
    ...
    // 方法1：优先用 begin_time 判断
    for (let i = phases.length - 1; i >= 0; i--) {
        const beginTime = toTimeSec(phases[i].begin_time);
        if (beginTime > 0 && nowSec >= beginTime) {
            if (debug) {
    ...
    // 方法2：如果 begin_time 都<=0（数据有问题），用倒计时备用
    const durations = getPlantPhaseDurations(plantId);
    if (durations && durations.length > 0 && growSec > 0) {
        ...
        // 用 growSec 和总时长估算当前阶段
        if (totalDuration > 0 && matureTime > 0) {
            ...
            if (remainingTime > 0) {
                ...
                for (let i = 0; i < durations.length; i++) {
                    ...
                    if (usedTime < accumulatedSec) {
                        return { phase: i + 1, begin_time: phases[i]?.begin_time || 0 };
                    }
                }
            } else {
                // 已成熟，返回成熟阶段
                return { phase: 6, begin_time: matureTime };
            }
        }
    }
...
async function checkFarm() {
    ...
    try {
        ...
        if (CONFIG.autoFarm && status.harvestable.length > 0) {
            try {
                if (CONFIG.harvestDelay > 0) {
                    await sleep(CONFIG.harvestDelay * 1000);
                }
                ...
                if (refreshedReply.lands && refreshedReply.lands.length > 0) {
                    ...
                    for (const hid of harvestedLandIds) {
                        if (refreshedStatus.empty.includes(hid)) {
                            allEmptyLands.push(hid);  // 空地 → 种植
                        } else if (refreshedStatus.dead.includes(hid)) {
                            allDeadLands.push(hid);    // 枯死 → 铲除
                        }
