...
function getLandStatus(land) {
    if (!land.unlocked) return 'locked';
    if (!land.plant) return 'empty';
    const phase = land.plant.phases?.[0]?.phase || 0;
    if (phase === 7) return 'dead';
    if (phase === 6) return 'ready';
    if (toNum(land.plant.dry_num) > 0) return 'needsWater';
...
function serializeLands(landsReply) {
    const accountId = reporter.getAccountId();
    ...
    return {
        ...
        lands: lands.map(land => {
            ...
            const currentPhase = plant?.phases ? getCurrentPhase(plant.phases, false, '', plantId, growSec) : null;
            ...
            // 如果没有phases数据，使用估算：假设刚种植，用growSec估算成熟时间
            // 这是一个粗略的估算，实际应该根据生长阶段计算
            if (!matureTime && plant && plant.grow_sec > 0) {
            ...
            return {
                ...
                plant: plant ? {
                    ...
                    phase: phase,
                    phaseName: getPhaseName(phase, Number(plant.id)),
...
function getPhaseName(phase, plantId) {
    const phaseToName = plantId ? getPlantPhaseNames(plantId) : null;
    if (phaseToName && phaseToName[phase]) {
        return phaseToName[phase];
    }
    const names = ['未知', '种子', '发芽', '小叶', '大叶', '开花', '成熟', '枯死'];
    return names[phase] || '未知';
}

async function reportLandsData(landsReply) {
    const data = serializeLands(landsReply);
    reporter.saveLandsJson(data);
    reporter.reportLands(data);
}
 ...
 * 使用倒计时计算当前阶段
 */
function getCurrentPhase(phases, debug, landLabel, plantId, growSec) {
    ...
    // 使用倒计时计算当前阶段
    const nowSec = getServerTimeSec();
    if (matureTime && matureTime > nowSec) {
        // 剩余时间
        const remainingTime = matureTime - nowSec;
        // 已用时间 = 总时长 - 剩余时间
        const usedTime = totalDuration - remainingTime;
        
        if (usedTime <= 0) {
            return { phase: 1, begin_time: phases[0]?.begin_time || 0 };
        }
        
        // 根据已用时间计算当前阶段
        let accumulatedSec = 0;
        for (let i = 0; i < durations.length; i++) {
            accumulatedSec += durations[i];
            if (usedTime < accumulatedSec) {
                return {
                    phase: i + 1,
                    begin_time: phases[i]?.begin_time || 0
                };
            }
        }
    }
    ...
    // 如果没有成熟时间或已过期，返回成熟阶段
    return { phase: 6, begin_time: phases[phases.length - 1]?.begin_time || 0 };
}

function analyzeLands(lands) {
    const result = {
        harvestable: [], needWater: [], needWeed: [], needBug: [],
        growing: [], empty: [], dead: [], locked: [],
        harvestableInfo: [],  // 收获植物的详细信息 { id, name, exp }
        harvestableHasNext: [],  // 收获后还有下一季的土地
    };
    ...
    for (const land of lands) {
        ...
        const growSec = toNum(plant.grow_sec);
        ...
        const currentPhase = getCurrentPhase(plant.phases, debug, landLabel, plantId, growSec);
        if (!currentPhase) {
            result.empty.push(id);
            continue;
        }
        ...
        if (phaseVal === PlantPhase.MATURE) {
            ...
            // 检查收获后是否还有下一季
            const currentPhaseIndex = plant.phases.findIndex(p => p.phase === phaseVal);
            const hasNextPhase = currentPhaseIndex >= 0 && currentPhaseIndex < plant.phases.length - 1;
            if (hasNextPhase) {
                result.harvestableHasNext.push(id);
            }
            // 收集植物信息用于日志
            const plantId = toNum(plant.id);
...
async function checkFarm() {
    ...
    try {
        ...
        let harvestedHasNext = [];  // 收获后还有下一季的土地
        if (CONFIG.autoFarm && status.harvestable.length > 0) {
            try {
                await harvest(status.harvestable);
                actions.push(`收获${status.harvestable.length}`);
                harvestedLandIds = [...status.harvestable];
                harvestedHasNext = [...status.harvestableHasNext];
                reporter.reportStats({ harvestCount: status.harvestable.length });
            } catch (e) { logWarn('收获', e.message); }
        }

        // 铲除 + 种植 + 施肥（需要顺序执行）- 受autoFarm和autoFertilize开关控制
        // 没有下一季的土地需要铲除+种植，有下一季的不做任何处理
        const toRemove = harvestedLandIds.filter(id => !harvestedHasNext.includes(id));  // 没有下一季，需要铲除
        const allDeadLands = [...status.dead, ...toRemove];
        const allEmptyLands = [...status.empty];  // 只包括真正的空地，不包括收获的土地
        if (CONFIG.autoFarm && (allDeadLands.length > 0 || allEmptyLands.length > 0)) {
            try {
        ...
        // 解锁土地 - 受 autoLandUnlock 开关控制
        if (CONFIG.autoLandUnlock && status.locked.length > 0) {
            try {
                await checkAndUnlockLand(lands);
            } catch (e) { logWarn('解锁土地', e.message); }
        }

        // 升级土地 - 受 autoLandUpgrade 开关控制
        if (CONFIG.autoLandUpgrade) {
            try {
                await checkAndUpgradeLand(lands);
...
function triggerFarmCheckNow() {
    if (!farmLoopRunning) return;
    if (farmCheckTimer) { clearTimeout(farmCheckTimer); }
    farmCheckTimer = setTimeout(() => farmCheckLoop(), 100);
}
...
async function unlockLand(landId) {
    try {
        const body = types.UnlockLandRequest.encode(
            types.UnlockLandRequest.create({ land_id: landId })
        ).finish();
...
async function upgradeLand(landId) {
    try {
        const body = types.UpgradeLandRequest.encode(
            types.UpgradeLandRequest.create({ land_id: landId })
        ).finish();
...
async function checkAndUnlockLand(lands) {
    for (const land of lands) {
...
async function checkAndUpgradeLand(lands) {
    for (const land of lands) {
        if (land.unlocked && land.could_upgrade) {
            try {
                await upgradeLand(land.id);
            } catch (e) { }
        }
    }
}
...
module.exports = {
    checkFarm, startFarmCheckLoop, stopFarmCheckLoop, triggerFarmCheckNow,
    getCurrentPhase,
