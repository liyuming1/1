...
const { CONFIG, PlantPhase, PHASE_NAMES, getPlantPhaseNames } = require('./config');
...
function serializeLands(landsReply) {
    ...
    return {
        ...
        lands: lands.map(land => {
            ...
            const currentPhase = plant?.phases ? getCurrentPhase(plant.phases) : null;
            ...
            return {
                ...
                plant: plant ? {
                    id: toNum(plant.id),
                    name: plant.name || getPlantNameBySeedId(toNum(plant.id)) || '未知',
                    phase: phase,
                    phaseName: getPhaseName(phase, toNum(plant.id)),
...
function getPhaseName(phase, plantId) {
    const plantPhaseNames = plantId ? getPlantPhaseNames(plantId) : null;
    if (plantPhaseNames && plantPhaseNames.length > 0) {
        return plantPhaseNames[phase] || `阶段${phase}`;
    }
...
async function autoPlantEmptyLands(deadLandIds, emptyLandIds, unlockedLandCount) {
    ...
    if (CONFIG.autoFertilize && plantedLands.length > 0) {
        // 如果开启购买化肥，先购买化肥
        if (CONFIG.autoBuyFertilizer) {
            try {
                await warehouse.buyFertilizer();
            } catch (e) { /* ignore if already has fertilizer */ }
        }
