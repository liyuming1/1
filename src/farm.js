...
async function autoPlantEmptyLands(deadLandIds, emptyLandIds, unlockedLandCount) {
    ...
    if (CONFIG.autoFertilize !== false && plantedLands.length > 0) {
        // 先普通肥
...
async function checkFarm() {
    ...
    try {
        ...
        if (CONFIG.autoFarm !== false && status.harvestable.length > 0) {
            try {
        ...
        if (CONFIG.autoFarm !== false && (allDeadLands.length > 0 || allEmptyLands.length > 0)) {
            try {
...
async function unlockLand(landId) {
    const { sendMsgAsync, types } = require('./network');
    try {
