...
function serializeLands(landsReply) {
    ...
    return {
        ...
        lands: lands.map(land => {
            ...
            // 备用：没有 phases 数据才用 grow_sec
            if (!matureTime && plant && plant.grow_sec > 0) {
                matureTime = serverTime + toNum(plant.grow_sec);
            }
            
            // 使用阶段累计时间点计算
            if (plant && plantId && matureTime) {
                const remainingSec = matureTime - serverTime;
                const totalGrowTime = getPlantGrowTime(plantId);
                const elapsed = totalGrowTime - remainingSec; // 已生长时间
                
                const durations = getPlantPhaseDurations(plantId);
                
                if (durations && durations.length > 0 && elapsed > 0) {
                    // 计算累计时间点
