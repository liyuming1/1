            ...
            try {
                ...
                if (lands.length > 0) {
                    ...
                    if (hostGid === userState.gid || hostGid === 0) {
                        ...
                        // 上报土地变化到 Manager
                        const changedData = lands.map(land => {
                            // 从phases中找到成熟阶段的时间戳
                            let matureTime = null;
                            const serverTime = Math.floor(Date.now() / 1000);
                            if (land.plant && land.plant.phases) {
                                for (const phase of land.plant.phases) {
                                    if (phase.phase === 6) {  // 6 = 成熟
                                        matureTime = toNum(phase.begin_time);
                                        break;
                                    }
                                }
                            }
                            ...
                            return {
                                ...
                                plant: land.plant ? {
                                    id: toNum(land.plant.id),
                                    name: land.plant.name || '',
                                    phase: land.plant.phases?.[0]?.phase || 0,
                                    matureTime: matureTime,
                                    growSec: toNum(land.plant.grow_sec),
                                    dryNum: toNum(land.plant.dry_num),
                                    hasWeed: (land.plant.weed_owners?.length || 0) > 0,
                        ...
                        reporter.reportLandsNotify(changedData);
                    }
                }
            } catch (e) { }
            return;
        ...
        if (type.includes('BasicNotify')) {
            try {
                ...
                if (notify.basic) {
                    ...
                    userState.voucher = toNum(notify.basic.voucher) || userState.voucher || 0;
