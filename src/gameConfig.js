    ...
    try {
        ...
        if (fs.existsSync(plantPath)) {
            ...
            for (const plant of plantConfig) {
                const plantId = plant.seed_id || plant.id;
...
function getLevelExpProgress(level, totalExp) {
    ...
    const nextLevelStart = levelExpTable[level + 1];
    
    // 已满级
    if (!nextLevelStart) {
        return { current: totalExp, needed: 0, maxed: true };
    }
...
function getPlantByName(name) {
    ...
    for (const plant of plantMap.values()) {
        if (plant.name && plant.name.replace(/\s+/g, '') === normalizedName) {
            return plant;
        }
    }
    return null;
 ...
 * 获取植物的生长阶段名称列表
 * @param {number} plantId - 植物ID
 * @returns {string[]} 阶段名称数组，如 ['种子', '发芽', '小叶子', '大叶子', '成熟']
 */
function getPlantPhaseNames(plantId) {
    const plant = plantMap.get(plantId);
