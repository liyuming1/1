...
function init() {
    ...
    configPath = path.join(accountDir, 'config.json');
    ...
    // 使用 fs.watchFile（比 fs.watch 更兼容 Linux）
    // 同时添加定时轮询作为备用方案
    
    // 方法1: fs.watchFile (更可靠)
    try {
        fs.watchFile(configPath, { interval: 1000 }, (curr, prev) => {
            if (curr.mtimeMs > lastMtime) {
                ...
                try {
                    ...
                    console.log('[ConfigWatcher] 读取新配置失败:', e.message);
                }
            }
        });
        console.log('[ConfigWatcher] 已启动文件监控 (watchFile)');
    } catch (e) {
        console.log('[ConfigWatcher] watchFile 失败，使用轮询方案:', e.message);
...
function applyConfig(newConfig) {
    ...
    if (newConfig.harvestDelay !== undefined) {
        const newDelay = Math.max(0, parseInt(newConfig.harvestDelay));
        if (CONFIG.harvestDelay !== newDelay) {
            CONFIG.harvestDelay = newDelay;
            changed = true;
            console.log(`[Config] 热更新: harvestDelay=${newDelay}s`);
        }
    }

    if (newConfig.stealDelay !== undefined) {
        const newDelay = Math.max(0, parseInt(newConfig.stealDelay));
        if (CONFIG.stealDelay !== newDelay) {
            CONFIG.stealDelay = newDelay;
            changed = true;
            console.log(`[Config] 热更新: stealDelay=${newDelay}s`);
        }
    }
