...
function buildRows(rawSeeds, lands, seedPhaseReduceMap) {
    ...
    for (const s of rawSeeds) {
        ...
        if (seasons === 2) {
            ...
            // 两季总周期 = 第一季 + 第二季
            cycleSecNoFert = growTimeSec1 + growTimeSec2;
            cycleSecNormalFert = growTimeNormalFert1 + growTimeNormalFert2;
            ...
            cycleSecNoFert = growTimeSec;
            cycleSecNormalFert = growTimeNormalFert;
    ...
    return { rows, skipped, missingPhaseReduceCount };
}
...
function analyzeExpYield(opts = {}) {
    ...
    const { rows, skipped, missingPhaseReduceCount } = buildRows(rawSeeds, lands, seedPhaseReduceMap);
...
function getExpRanking(level, lands, topN = 5) {
    const payload = analyzeExpYield({
        input: DEFAULT_INPUT,
        lands: lands == null ? 18 : lands,
        top: topN,
    ...
    const availableRows = payload.rows.filter(r => r.requiredLevel <= level);
    
    const topNoFert = pickTop(availableRows, 'farmExpPerHourNoFert', topN).map(r => ({
        seedId: r.seedId,
        name: r.name,
        requiredLevel: r.requiredLevel,
        expPerHour: Number(r.farmExpPerHourNoFert.toFixed(2)),
        growTime: r.growTimeStr,
    }));

    const topNormalFert = pickTop(availableRows, 'farmExpPerHourNormalFert', topN).map(r => ({
        seedId: r.seedId,
        name: r.name,
        requiredLevel: r.requiredLevel,
        expPerHour: Number(r.farmExpPerHourNormalFert.toFixed(2)),
        growTime: r.growTimeNormalFertStr,
        gainPercent: Number(r.gainPercent.toFixed(2)),
    }));
    ...
    return {
        ...
        topNoFert,
        topNormalFert,
        recommendedSeedId: topNormalFert.length > 0 ? topNormalFert[0].seedId : null,
        recommendedSeedName: topNormalFert.length > 0 ? topNormalFert[0].name : null,
    };
}
